...
var _setDownloadFilename = _interopRequireDefault(require("./set-download-filename"));
var _xlsx = require("xlsx/xlsx.mjs");
...
function _default(params) {
  function excel_sheet(columns, data) {
    ...
    let ws = _xlsx.utils.aoa_to_sheet(excel_data); // worksheet
    // larghezza colonne
    ws['!cols'] = [];
    columns.forEach(col => {
      ws['!cols'].push({
        wch: values_lengths[col.key] + 4
      });
    });
    return ws;
  }
  ...
  let wb = _xlsx.utils.book_new(); // workbook

  _xlsx.utils.book_append_sheet(wb, excel_sheet(params.columns, params.data), setSheetName(params.downloadFilename));
  if (params.extraDownloadSheets) {
    params.extraDownloadSheets.forEach(item => {
      _xlsx.utils.book_append_sheet(wb, _xlsx.utils.aoa_to_sheet(item.data), setSheetName(item.name));
    });
  }
  const filename = (0, _setDownloadFilename.default)(params.downloadFilename, params.downloadFormat);
